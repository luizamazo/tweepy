...
class API(object):
    ...
    def check_status(self, media_id, processing_info):
        ...
        while flag: 
            ...
            if data.processing_info['state'] == 'succeeded':
                ...
                err = 'deu ruim no processamento de media'
                flag = False 
                data = {'erro':'deu ruim no processamento de midia'}
        return result   
    ...
    def upload_chunked(self, filename, *args, **kwargs):
        ...
        if media_info.media_id:
            ...
            if hasattr(finalize, 'processing_info'):  
                media_id = finalize.media_id_string 
                processing_info = finalize.processing_info
                media_info = self.check_status(media_id, processing_info)
                return media_info
            else:
                return finalize
        else:
